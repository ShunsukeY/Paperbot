...
# ======================
#  Graphical Abstract (og:image)
# ======================

def get_og_image_url(start_url: Optional[str]) -> Optional[str]:
    """
    指定URLの最終到達先HTMLから og:image / twitter:image を探して返す。
    見つからなければ None。
    """
    if not start_url:
        return None
    ...
    try:
        ...
        logger.warning("Failed to resolve URL for og:image (%s): %s", start_url, e)
        return None

    try:
        r2 = requests.get(final_url, headers=COMMON_HEADERS, timeout=10)
        r2.raise_for_status()
    except Exception as e:
        logger.warning("Failed to fetch HTML for og:image (%s): %s", final_url, e)
        return None

    try:
        soup = BeautifulSoup(r2.text, "html.parser")
    except Exception as e:
        logger.warning("BeautifulSoup parse error (%s): %s", final_url, e)
        return None
    ...
    if not candidates:
        logger.info("No og:image/twitter:image found for %s", final_url)
        return None
...
def get_graphical_abstract_url_for_paper(paper: dict) -> Optional[str]:
    """
    論文情報から、og:image ベースの GA URL を推定して返す。
    DOI があれば https://doi.org/DOI を優先し、それ以外は paper["url"] を使う。
    """
    doi = paper.get("doi")
    if doi and doi != "(no DOI)":
        start_url = f"https://doi.org/{doi}"
    else:
        start_url = paper.get("url")

    return get_og_image_url(start_url)
...
def main():
    logger.info("=== paperbot Crossref+PubMed (multi-query, abstracts+types+JA, HTML mail + og:image) start ===")
    now = datetime.now()
    ...
    for query in SEARCH_QUERIES:
        ...
        # og:image ベースのグラフィカルアブストURL取得
        for p in ranked:
            ga_url = get_graphical_abstract_url_for_paper(p)
            p["ga_image_url"] = ga_url
        ...
        if not ranked:
            ...
            for i, p in enumerate(ranked, start=1):
                ...
                if p.get("ga_image_url"):
                    lines.append(f"    GA image (og:image): {p['ga_image_url']}")
        ...
        if not ranked:
            ...
            for i, p in enumerate(ranked, start=1):
                ...
                ga_url = p.get("ga_image_url")
                if ga_url:
                    esc_ga = escape(ga_url)
                    html_lines.append(
                        f'<br>GA image: <a href="{esc_ga}">{esc_ga}</a><br>'
                        f'<img src="{esc_ga}" alt="Graphical abstract" '
                        f'style="max-width: 400px; border: 1px solid #ccc; margin-top:4px;"><br>'
                    )
